...
function Horizel(props) {
 return (  <div onClick={()=>{props.pager(props.id)}} style={{display:"flex",padding:10,justifyContent:"center",flexDirection:"column",alignItems:'center',color:MAIN_STYLE.primary,backgroundColor:props.current==props.id?"lightgrey":"white"}}>
 <props.icon  onClick={()=>{}} style={{fontSize:29,cursor:"pointer"}} />
   <div style={{fontWeight:"bold",fontSize:13,color:MAIN_STYLE.grey}} > {props.text} </div>
 </div>)

//<div  onClick={()=>{props.pager(props.id)}} className='transition ease-in-out rounded-none lg:rounded-md xl:rounded-md  hover:bg-gray hover:text-white' style={{display:"flex",flexDirection:'row',alignItems:'center',padding:10,paddingTop:15,paddingBottom:15,cursor:"pointer",backgroundColor:props.current==props.id?"lightgrey":"white",marginTop:10}}>
